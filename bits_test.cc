...
void test_limits(const char* min_chk, const char* max_chk)
{
    ...
    bool is_signed = bits::type_min<T>() < T(0);
    if (is_signed) {
        snprintf(min_str, sizeof(min_str), "%lld", (ull64)bits::type_min<T>());
        snprintf(max_str, sizeof(max_str), "%lld", (ull64)bits::type_max<T>());
    } else {
        snprintf(min_str, sizeof(min_str), "%llu", (ull64)bits::type_min<T>());
        snprintf(max_str, sizeof(max_str), "%llu", (ull64)bits::type_max<T>());
    }
    ...
    printf(is_signed ? print_fmt_signed : print_fmt_unsigned,
           ...
           (ll64)bits::type_min<T>(), (ull64)bits::type_min<T>(), min_chk);
    printf(is_signed ? print_fmt_signed : print_fmt_unsigned,
           max_pass ? "PASS" : "FAIL", __func__, "max",
           (ll64)bits::type_max<T>(), (ull64)bits::type_max<T>(), max_chk);
}

template <typename T>
void test_binary_string(T val, const char* chk)
{
    bool pass = strcmp(bits::to_binary_string<T>(val).c_str(), chk) == 0;
    printf("%s %-24s val=%-20llu (0x%016llx) chk=%s\n",
           pass ? "PASS" : "FAIL", __func__,
           (ull64)val, (ull64)val, chk);
}
...
void test_sub_underflow(T a, T b, bool expected, int iters = TIME_ITERATIONS)
{
    ...
    bool underflow = bits::sub_underflow<T>(a, b);
...
void test_popcount(T val, int expected, int iters = TIME_ITERATIONS)
{
    ...
    int count = bits::popcount<T>(val);
    bool pass = count == expected;
    printf("%s [%4llu,%4llu,%4llu] %-24s val=%-20llu (0b%s) count=%d expected=%d\n",
           pass ? "PASS" : "FAIL", (ull64)tmin, (ull64)tmax, (ull64)tavg, __func__,
           (ull64)val, bits::to_binary_string<T>(val).c_str(), count, expected);
}
...
void test_ispow2(T val, bool expected, int iters = TIME_ITERATIONS)
{
    ...
    bool pow2 = bits::ispow2<T>(val);
    bool pass = pow2 == expected;
    printf("%s [%4llu,%4llu,%4llu] %-24s val=%-20llu (0b%s) pow2=%d expected=%d\n",
           pass ? "PASS" : "FAIL", (ull64)tmin, (ull64)tmax, (ull64)tavg, __func__,
           (ull64)val, bits::to_binary_string<T>(val).c_str(), pow2, expected);
}
...
void test_roundpow2(T val, int expected, int iters = TIME_ITERATIONS)
{
    ...
    int rounded = bits::roundpow2<T>(val);
    bool pass = rounded == expected;
    printf("%s [%4llu,%4llu,%4llu] %-24s val=%-20llu (0b%s) rounded=%d expected=%d\n",
           pass ? "PASS" : "FAIL", (ull64)tmin, (ull64)tmax, (ull64)tavg, __func__,
           (ull64)val, bits::to_binary_string<T>(val).c_str(), rounded, expected);
}
